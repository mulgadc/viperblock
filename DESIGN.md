...
**The snapshot-ancestry hazard.** Snapshots do not copy data — `CreateSnapshot` freezes a checkpoint that references the source volume's existing chunks in place, and clone reads fetch those chunks directly from the source volume's prefix. A GC that only consults the live map would delete exactly the chunks a snapshot (or any clone/AMI descended from it) still needs the moment the source volume supersedes them — silent, cross-volume data loss. Three guards close this:
...
- **Per-volume snapshot marker.** `CreateSnapshot` writes `{volumeName}/snapshots.marker`, a small change token naming the most recent snapshot of the volume, and writes it *before* it reads the map it will freeze. Every sweep re-reads that one object and compares it to the baseline captured alongside the ancestry scan; any difference latches GC off permanently. This covers the case the scan cannot: a snapshot created by a **different process** after the scan cached "safe" — which is spinifex's primary path, since the NBD plugin serves and sweeps an attached volume while the daemon snapshots it for `CreateImage`. Cost is one GET per sweep, constant in the number of snapshots in the deployment, versus the ancestry scan's O(total snapshots).

  The ordering is what makes it sound, and it is why the check sits immediately before the deletes rather than at the top of the sweep. A snapshot is only endangered if it freezes a map *older* than the live checkpoint the sweep's candidates were derived from; a snapshot freezing the same or a newer map cannot name a swept chunk at all, since a chunk absent from the live map never returns to it. In that dangerous case the snapshot wrote its marker before it read that older map, which is before the sweep's checkpoint was durable, which is before the sweeper reads the marker — so the sweeper always observes the change. A marker write failure fails the snapshot, and a marker read failure skips the sweep: both fail towards under-collection, never towards deletion.

The existence scan is deliberately conservative and does not attempt to distinguish "a snapshot exists but doesn't reference the chunks about to be swept" — any existing snapshot of the volume disables GC for it outright. `GCEnabled` remains opt-in per volume.
...
`Reset()` (which re-issues `ObjectID` 0) and `CreateSnapshot` both latch GC off permanently for the instance they run on, since both invalidate an invariant the design depends on (ID reuse, and snapshot ancestry, respectively). A moved snapshot marker latches the same flag, which is how a snapshot taken in another process reaches this instance.
